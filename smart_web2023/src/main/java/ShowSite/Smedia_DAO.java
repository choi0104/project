...
public class Smedia_DAO extends S_DBconnection {
	...
	public int insert(Sfree_DTO dto) {
		...
		try {
			if(dto.getOriginfile_name() != null) {
				...
				String sql = "insert into SMEDIA_BOARD (b_idx, s_idx, writer, title, content, originfile_name, savefile_name) "
				...
				String sql = "insert into SMEDIA_BOARD (b_idx, s_idx, writer, title, content) "
						     + " values(board_seq.nextval, ?,?,?,?)";
	...
	public ArrayList<Sfree_DTO> getBoards(){
		...
		String sql = "select * from SMEDIA_BOARD "
				+ " where del_or_not = 1 "
				+ " order by b_idx desc";
	...
	public void updateRead_cnt(int b_idx) {
		...
		String sql = "update SMEDIA_BOARD set read_cnt = (read_cnt + 1) where b_idx = ?";
	...
	public Sfree_DTO getBoard(int b_idx){
		...
		String sql = "select * from SMEDIA_BOARD where b_idx = ?";
	...
	public int update(Sfree_DTO dto) {
		...
		try {
			if(dto.getOriginfile_name() != null) {
				...
				String sql = "update SMEDIA_BOARD set title=?, content=?, originfile_name=?, savefile_name=? "
						     + " where b_idx=?";
				...
				String sql = "update SMEDIA_BOARD set title=?, content=? "
					     + " where b_idx=?";
	...
	public int delete(int b_idx) {
		...
		try {
			...
			String sql = "update SMEDIA_BOARD set del_or_not=-1 "
				     + " where b_idx=?";
